...
public class TripletWithMaxSum {
	...
	public TripletWithMaxSum(int[] arr) {
		// TODO Auto-generated constructor stub
		int maxTripletSumIndex=0;
		int max=0;
		for(int i=0, j=0; i<arr.length-2; i++) {
			if(arr[i+1]==arr[i]+1 && arr[i+2]==arr[i]+2) {
				if(max<arr[i]) {
					max=arr[i];
					maxTripletSumIndex=i;
				}		
				System.out.println(arr[i]+","+arr[i+1]+","+arr[i+2]);
			}
		}
		System.out.println("The triplet with maximum sum is: "+arr[maxTripletSumIndex]+","+arr[maxTripletSumIndex+1]+","+arr[maxTripletSumIndex+2]);
	}
	
	public static void main(String[] args) {
		int[] input = {1,3,4,5,7,3,9,10,11,12,44,67,1,2,3,99};
		new TripletWithMaxSum(input);
	}
}