...
public class PickMaxRepeatingWordsInStringV1 {
	//The program for displaying first max repeating word if there is a tie between two words.
	void pickMaxFrequency(String s) {
		...
		for(int i=0; i<s1.length; i++) {
			...
			for(int j=i+1; j<s1.length; j++) {
				if(s1[i].equals(s1[j]))
					count++;
				if(count>maxCount) { //here the condition is '>' which displays first word during tie situation.
					maxCount = count;
					maxRepeatWord = s1[i];
				}
			}
	...
	public static void main(String[] args) {
		...
		new PickMaxRepeatingWordsInStringV1().pickMaxFrequency(str);		
	}
}