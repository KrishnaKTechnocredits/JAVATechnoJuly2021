...
public class SumAfterTriplet {
	...
	void getZeroAfterTriplet(int arr[]) {
		...
		for (; index < arr.length - 2; index++, count++) {
			...
			if ((arr[index + 2] == arr[index] + 2) && (arr[index + 1] + 1 == arr[index + 2])) {
				...
				output[++count] = arr[index] + arr[index + 1] + arr[index + 2];
				index += 2;
	...
	public static void main(String[] args) {
		...
		SumAfterTriplet sumAfterTriplet = new SumAfterTriplet();
		sumAfterTriplet.getZeroAfterTriplet(input);
	}
}
